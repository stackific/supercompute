...
| Playbook | Purpose |
| --- | --- |
| `playbooks/wireguard-up.yml` | Classify static/roaming; hub; Mac controller when `control_plane!=gha`; nodes; Mac mesh proof when `control_plane!=gha` |
...
5. **localhost** — Prove Mac controller mesh SSH to every node when `control_plane` is not `gha` (GHA proves after `gha-mesh-peer`).
6. **nodes** — Prove every server can reach every peer through the mesh (Ansible SSH uses bootstrap recovery when the controller is off-mesh; `wait_for` still checks peer mesh IPs on each node).
