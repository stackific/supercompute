...
- `prod_ssh_host_ed25519_sha256` — `SHA256:…` from the VM
- `prod_wireguard_address` — mesh IP in `10.217.80.0/24` (default `.11`)

Set `prod_default_ssh_user` / `prod_ssh_private_key_file` in
...
`lima-up` auto-fills `prod_ssh_host_ed25519_sha256` for every
...
`prod_ssh_host_ed25519_sha256` yourself, same as [setup-prod.md](setup-prod.md).
Do **not** set `prod_bootstrap_ssh_host` or `prod_wireguard_endpoint` on Lima
guests.
...
1. Prepare public `static-1` and fill inventory placeholders (including its
   fingerprint by hand).
2. `task lima-up PROVIDER=dev` — only `node_lima_guest` hosts; auto-fills their
