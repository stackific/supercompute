...
### Step 3: Operator SSH key for prod

Create an unpassphrased operator key for the GitHub runner and install the public key on both VMs.

Create a **separate** key from dev. Use an **empty passphrase**—the runner cannot unlock a passphrase-protected key. Store both files in your password manager.
...
ssh-add ~/.ssh/example-prod
pbcopy < ~/.ssh/example-prod.pub
```
...
ssh-add ~/.ssh/<project>-prod
pbcopy < ~/.ssh/<project>-prod.pub
```

</TabItem>
</Tabs>

Inventory resolves this path automatically via `ssh_private_key_file` in group vars (`~/.ssh/{{ project }}-{{ inventory_slug }}` → `~/.ssh/example-prod` when `project: example` and `ENV=prod`).

Install the **public** key on each production VM (`ops` user)—follow [Adding a public node](/guides/adding-public-node/) for each host. The **private** key is what you store later as GitHub secret `OPS_SSH_PRIVATE_KEY`. The workflow writes that secret to `~/.ssh/${project}-${ENV}` on the runner—the same path Ansible expects.

### Step 4: Set GHA control plane
