...
Edit `inventories/prod/hosts.yml` the same way as [Deploy to production — Step 1](/start-here/deploy-to-production/#step-1-configure-prod-hosts): set the `sc_*` FQDNs, then fill `public_ip` and `ssh_ed25519_sha256` for **static-1** and **static-2**. Install this public key on each VM with static IP (`ops` user)—same prep as [Adding a public node](/guides/adding-public-node/). Go and do that first.
...
Supercompute's Nameserver then serves `*.sc-apps.example.com`. If the VM IPs are not ready yet, add the A records before bring-up.
...
5. Firewall: TCP **22** open for bootstrap (runner egress IPs change; plan accordingly). Open **UDP 51830** broadly if you add roaming nodes later. The first static hub also needs inbound **TCP 2333** for rathole ([Adding a roaming node](/guides/adding-roaming-node/)).
...
3. Validate `hosts.yml` (`sc_*`) and vault (`vault_database_url`, `vault_database_secret`).
