...
| | Mac (`control_plane: mac`) | GHA (`control_plane: gha`) |
| --- | --- | --- |
...
Edit `inventories/prod/hosts.yml` the same way as [Deploy to production — Step 1](/start-here/deploy-to-production/#step-1-configure-prod-hosts): set `hostname`, then fill `public_ip` and `ssh_ed25519_sha256` for **static-1** and **static-2**.
