...
The example uses the **prod environment**, host name **`roaming-1`**, and hostname **`roaming-1.sc.example.com`**.
...
| Field | Example | Notes |
| --- | --- | --- |
| Subdomain | `roaming-1` | Becomes `roaming-1.sc.example.com` |
| Domain | `sc.example.com` | Must be a zone in this account |
| Path | *(leave empty)* | |
...
Cloudflare usually creates a **proxied CNAME** for `roaming-1.sc.example.com` → `<tunnel-id>.cfargotunnel.com`. Confirm under your zone **DNS** → **Records**.

If DNS is missing, from a machine with tunnel credentials:

```sh
cloudflared tunnel route dns home-lab-ssh roaming-1.sc.example.com
```
...
3. **Application domain**: `roaming-1.sc.example.com` (same hostname as Step 4).
...
| DNS record for `roaming-1.sc.example.com` | |
| Access application allows your identity | |
| `ssh roaming-1.sc.example.com true` from Mac | |
