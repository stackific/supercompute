...
To add another public node, duplicate the block as `static-2`, assign the next mesh IP (e.g. `.12`), add an A record for `sc_ns` pointing at that node's public IPv4, and run `task up ENV=<env>` again. If the DNS is hosted on Cloudflare, do not enable the proxy orange icons.

### Step 7: Cloud provider firewall

Open TCP 22 for bootstrap, UDP 51830 if roaming peers will dial in later, and TCP 2333 on the first static hub for rathole.
...
| After setup | On the **first static hub**, allow inbound **TCP 2333** if you will add a non-Lima roaming node ([Adding a roaming node](/guides/adding-roaming-node/)) |
