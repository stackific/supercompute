...
Requires `task setup` first (SHA-pinned binary in `.vendor/rathole/`). Writes `.state/<env>/rathole/<host>-install.sh`. Copy that script to the roaming VM and run it as root (console or LAN SSH), then [Adding a roaming node](/guides/adding-roaming-node/). Lima guests (`node_lima_guest`) do not use this task.
