...
Requires `task setup` first (SHA-pinned binary in `.vendor/rathole/`). Writes `.state/<env>/rathole/<host>-install.sh`. Follow [Adding a roaming node](/guides/adding-roaming-node/) to open hub TCP **2333**, copy the script to the VM, run it as root, and prove jump SSH. Then `task up` (Mac control plane) or Actions Deploy `up` (`control_plane: gha`). This task is required once even for GHA inventories; commit `vault.yml` if it added rathole secrets. Lima guests (`node_lima_guest`) do not use this task.
