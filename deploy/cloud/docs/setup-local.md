...
with Lima on Apple Silicon. They share the same inventory layout as other
providers under `inventories/<slug>/`.
...
- WireGuard tools (`wg`, `wg-quick`) for the mesh commands
...
and the command never prompts for one.
...
```sh
task vault-edit PROVIDER=local
```
